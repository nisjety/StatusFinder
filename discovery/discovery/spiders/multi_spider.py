...
The spider implements efficient handling of async Playwright operations within Scrapy's synchronous framework:
- Uses asyncio event loops to handle async Playwright operations
- Properly manages page lifecycle (load state monitoring and cleanup)
- Ensures clean teardown of Playwright resources
- Maintains sync interface compatibility with BaseSpider
    ...
    def parse(self, response):
        ...
        if 'playwright_page' in response.meta:
            ...
            try:
                ...
                # Run the async page handling in the event loop
                loop = asyncio.get_event_loop()
                loop.run_until_complete(handle_page())
                
            except Exception as e:
                self.logger.error(f"Playwright error: {str(e)}")
            finally:
                if page:
                    # Close the page correctly using the Playwright async API
